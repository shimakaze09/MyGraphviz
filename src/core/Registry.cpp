...
size_t Registry::GetEdgeIndex(std::string_view lhsID,
                              std::string_view rhsID) const {
  size_t lhs = GetNodeIndex(lhsID);
  size_t rhs = GetNodeIndex(rhsID);
...
Registry& Registry::RegisterNodeAttr(size_t nodeIndex, std::string key,
                                     std::string value) {
  nodeAttrs[nodeIndex].emplace(std::move(key), std::move(value));
  return *this;
}

Registry& Registry::RegisterEdgeAttr(size_t edgeIndex, std::string key,
                                     std::string value) {
