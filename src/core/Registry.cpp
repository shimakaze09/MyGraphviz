...
bool Registry::IsRegisteredEdge(const std::string& lhsID,
...
size_t Registry::GetEdgeIdx(const std::string& lhsID,
                            const std::string& rhsID) const {
  size_t lhs = GetNodeIdx(lhsID);
  size_t rhs = GetNodeIdx(rhsID);
...
Registry& Registry::RegisterNodeAttr(size_t nodeIdx, std::string key,
                                     std::string value) {
  nodeAttrs[nodeIdx].emplace(std::move(key), std::move(value));
  return *this;
}

Registry& Registry::RegisterEdgeAttr(size_t edgeIdx, std::string key,
                                     std::string value) {
