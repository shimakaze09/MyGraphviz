...
bool Registry::IsRegisteredEdge(std::string_view lhsID,
                                std::string_view rhsID) const {
...
Registry& Registry::RegisterNodeAttr(std::size_t nodeIndex, std::string key,
                                     std::string value) {
  nodeAttrs[nodeIndex].emplace(std::move(key), std::move(value));
  return *this;
}

Registry& Registry::RegisterEdgeAttr(std::size_t edgeIndex, std::string key,
                                     std::string value) {
  edgeAttrs[edgeIndex].emplace(std::move(key), std::move(value));
  return *this;
}

Registry& Registry::DeregisterNodeAttr(std::size_t nodeIndex,
                                       std::string_view key) {
  nodeAttrs[nodeIndex].erase(nodeAttrs[nodeIndex].find(key));
  return *this;
}

Registry& Registry::DeregisterEdgeAttr(std::size_t edgeIndex,
                                       std::string_view key) {
