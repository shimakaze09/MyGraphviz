...
class Registry {
 public:
  ...
  std::size_t GetEdgeIndex(std::string_view lhsID,
                           std::string_view rhsID) const;

  std::size_t RegisterNode(std::string ID);
  std::size_t RegisterEdge(std::size_t lhs, std::size_t rhs);

  Registry& RegisterNodeAttr(std::size_t nodeIndex, std::string key,
                             std::string value);
  Registry& RegisterEdgeAttr(std::size_t edgeIndex, std::string key,
                             std::string value);
 ...
 private:
  ...
  std::map<std::size_t, std::map<std::size_t, std::size_t>, std::less<>>
      node2edge;
